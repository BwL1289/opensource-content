...
class EquityEquation:
    ...
    def estimated_valuation_increase(self, estimated_valuation_increase: Any) -> None:
        ...
        if (
            estimated_valuation_increase <= Constants.ZERO.value
            or estimated_valuation_increase >= Constants.ONE_HUNDRED.value
        ):
            msg = f"""Invalid value for equity percentage.\
    ...
    def _setup_excel_workbook(self) -> tuple[Worksheet, Workbook]:
        ...
        total_equity_pool = float(
            input(
                """Enter the total equity pool as a
                                            percentage
                                            (total_equity_pool, between 0 and 100%): """
            )
        )
        ...
        if proposed_equity_alloc < Constants.ZERO.value or proposed_equity_alloc >= Constants.ONE_HUNDRED.value:
            msg = """Invalid value for proposed equity percentage. It should be greater than 0 and less
                than 100 (excluding 100)."""
            logger.error(msg)
            raise ValueError(msg)

        if estimated_valuation_increase <= Constants.ZERO.value:
            msg = "Invalid value for estimated valuation increase. It should be greater than 0."
            logger.error(msg)
            raise ValueError(msg)

        if total_equity_pool <= Constants.ZERO.value or total_equity_pool > Constants.ONE_HUNDRED.value:
            msg = "Invalid value for estimated valuation increase. It should be greater than 0 and less than 100."
            logger.error(msg)
            raise ValueError(msg)

        if pre_money_valuation <= Constants.ZERO.value:
            msg = "Invalid value for company valuation. It should be greater than 0."
            logger.error(msg)
            raise ValueError(msg)
...
if __name__ == "__main__":
    eeq = EquityEquation(
        total_equity_pool=100,
        proposed_equity_alloc=7,
        estimated_valuation_increase=7.5,
